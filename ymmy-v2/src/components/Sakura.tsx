...
export default function Sakura({ bg, theme, switchTheme }: SakuraProps) {
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      sizeRef.current = { width: viewport.offsetWidth, height: viewport.offsetHeight };
    };
    updateSize();
    window.addEventListener('resize', updateSize);

    const makeWindSpeed = (magnitude: number, duration: number, height: number) =>
      (t: number, y: number) => {
        const a = ((magnitude / 2) * (height - (2 * y) / 3)) / height;
        return a * Math.sin(((2 * Math.PI) / duration) * t + (3 * Math.PI) / 2) + a;
      };
    ...
    const resetLeaf = (leaf: Leaf) => {
      ...
      if (leaf.x > width) { leaf.x = width + 10; leaf.y = (Math.random() * height) / 2; }
    ...
    const tick = () => {
      ...
      for (const leaf of leavesRef.current) {
        ...
        if (leaf.x < -10 || leaf.y > sizeRef.current.height + 10) resetLeaf(leaf);
        void width;
      }
    ...
    <Parallax strength={200} bgImage={bg ?? undefined}>
      ...
      >
        ...
        <div className='headbox'>
          <h1 className='headbox__name'>Oleg Nosyrev</h1>
          <p className='headbox__title'>Software Engineer (Frontend-Leaning Full Stack)</p>
          <a className='headbox__resume' href={resume} target='_blank' rel='noreferrer'>
            ↓ Resume
          </a>
        </div>
